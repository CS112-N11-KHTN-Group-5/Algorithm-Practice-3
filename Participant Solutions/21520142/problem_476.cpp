...
// intersection point between segment ab and segment cd assuming unique
// intersection may not exists se.size()==0 means no intersection se.size()==1
// means one intersection se.size()==2 means range intersection
...
int main(int argc, char const *argv[]) {
  ...
  while (T--) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      h /= 2.0;
      w /= 2.0;
      vector<PT> rect_pts = {PT(-w, h), PT(w, h), PT(w, -h), PT(-w, -h)};
      for (auto &pt : rect_pts) {
        pt = rotatecw(pt, deg_to_rad(phi));
        pt = pt + PT(x, y);
        pts.push_back(pt);
      }
    }
    ...
    cout << setprecision(2) << fixed << rects_area / hull_area * 100.0 << "\n";
  }
  return 0;
}